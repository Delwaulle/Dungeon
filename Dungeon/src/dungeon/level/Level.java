...
public class Level {
	...
	public void initializeRooms() {
		...
		intersection
				.setDescription("There might be something weird on the left...");
	...
	public void initializeRooms2() {
		...
		intersection = new NormalRoom("intersection", this);
		intersection
				.setDescription("2 ways, west or east. I can hear something strange on the east...");
	...
	public void generateLevel(int numLevel) {
		...
		int aRandomRoom;
		Direction direction;
		int[] tabWithAllDirectionsTaken = new int[4];
		...
		while (roomsLeft > 0) {
			...
			for (int i = 0; i < nbDoor; i++) {
				...
				direction = generateADireciton(tabWithAllDirectionsTaken);
				...
				((Room) currentRoom).setNeighbour(new Door(direction),
						(Room) nextRoom);
	...
	private Direction generateADireciton(int[] tabWithAllDirectionsTaken) {
	...
	public void interpretCommand(String command) {
		...
		case "use":
			...
			if (cmd.length != 2)
				...
				potionName = cmd[1];
			commandFactory.setCommand(new ConsumeHealPotionCommand(this.player,
					potionName));
			commandFactory.invoke();
		...
		case "equip":
			...
			if (cmd.length != 2)
				...
				equipName = cmd[1];
			commandFactory.setCommand(new EquipItemCommand(this.player,
					equipName));
			commandFactory.invoke();
		...
		default:
			System.out
					.println("I don't know what you mean. Type \"help\" to show the differents commands.");
		}
	}
	...
	/**
	 * the buckle into which one the player is into for a level we give him the
	 * possible directions from his current room and we made it until he
	 * completes the level or loses the game
	 * 
